...
export function Review() {
  ...
  async function decide(prospectId: Id<"prospects">, decision: "confirm_hold" | "decline" | "proceed_with_consent") {
    setBusy(prospectId);
    try {
      await review({ prospectId, reviewer, decision, note: note[prospectId] ?? "" });
  ...
  return (
    <div className="review">
      ...
      <label className="field inline"><span className="meta">Reviewing as</span><input value={reviewer} onChange={(e) => setReviewer(e.target.value)} /></label>
      ...
      {!queue ? <Empty>Loading…</Empty> : queue.length === 0 ? (
        ...
        <section key={q.prospectId} className="band case appear" aria-labelledby={`case-${q.prospectId}`}>
          <div className="band-head">
            <h2 id={`case-${q.prospectId}`}><a href={`#/record/${q.prospectId}`}>{q.subject ?? "(no subject)"}</a><span className="meta">{q.from}</span></h2>
            <Verdict v={q.verdict} />
          </div>
          ...
          <div className="actions">
            <button type="button" className="btn ghost" disabled={busy === q.prospectId} onClick={() => decide(q.prospectId, "confirm_hold")}><PauseCircle size={15} strokeWidth={1.75} aria-hidden />Confirm hold</button>
            <button type="button" className="btn ghost" disabled={busy === q.prospectId} onClick={() => decide(q.prospectId, "decline")}><Ban size={15} strokeWidth={1.75} aria-hidden />Decline instruction</button>
            <button type="button" className="btn ghost" disabled={busy === q.prospectId} onClick={() => decide(q.prospectId, "proceed_with_consent")}><Check size={15} strokeWidth={1.75} aria-hidden />Proceed with informed consent</button>
          </div>
        </section>
      ))}
    </div>
